...
const CartStep = memo(({ step, className, ...props }) => {
  ...
  return (
    <div className={[styles.flex, styles.justify_center, className].join(' ')} {...props}>
      <Text role='title'>{t('user.cart.progress-input')}</Text>
      <Text role='title' className={step == 1 ? styles.disable : ''} style={{ 'margin': '0 8px' }}>▶</Text>
      <Text role='title' className={step == 1 ? styles.disable : ''}>{t('user.cart.progress-confirm')}</Text>
      <Text role='title' className={step == 1 || step == 2 ? styles.disable : ''} style={{ 'margin': '0 8px' }}>▶</Text>
