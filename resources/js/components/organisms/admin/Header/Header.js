          ...
          <nav>
            <ul className={[styles.header].join(' ')}>
              <li onClick={() => setOpenAdminMenu(!openAdminMenu)} className={styles.mr_auto}>
                <Icon
                  src={openAdminMenu ? "/img/close_icon_white.svg" : "/img/menu_icon.svg"}
                  className={[styles.icon, styles.meun_icon].join(' ')}
                />
              </li>
              {isAdminLogin &&
                <>
              ...
              <li className={styles.relative}>
                <Text role='reverse' className={styles.lang} onClick={() => setOpenPulldown(!openPulldown)}>
                  {localStorage.getItem('lang') ? localStorage.getItem('lang').toUpperCase() : 'EN'}
                </Text>
                {openPulldown &&
              ...
              {isAdminLogin ? (
                <li
                  onClick={() => handleLogout({ url: `/api/admin/logout`, callback: () => setIsAdminLogin(false) })}
                  className={[styles.text, styles.ml].join(' ')}
                >
