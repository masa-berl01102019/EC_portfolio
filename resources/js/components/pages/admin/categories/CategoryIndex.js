              ...
              {categories && categories.map((category) =>
                ...
                <li key={category.id} className={styles.mb_24}>
                  {category.id === editableForm ? (
                    ...
                    <div className={[styles.category_text, styles.text_gender].join(' ')} onClick={() => setEeditableForm(category.id)}>
                  ...
                  <ul>
                      ...
                      <li key={child.id}>
                        {child.id === editableForm ? (
                          ...
                          <>
                            ...
                            <div className={[styles.category_text, styles.text_main].join(' ')} onClick={() => setEeditableForm(child.id)}>
                              {child.category_name}
                            </div>
                          </>
                        )
                        ...
                        <ul className={styles.hidden}>
                            ...
                            <li key={grand_child.id} className={styles.pl_32}>
                              {grand_child.id === editableForm ? (
                                ...
                                <div className={[styles.flex, styles.align_center, styles.mb_8].join(' ')}>
                                  ...
                                  <div className={[styles.category_text, styles.text_sub].join(' ')} onClick={() => setEeditableForm(grand_child.id)}>
                          ...
                          <li className={[styles.flex, styles.pl_64, styles.mb_8].join(' ')}>
                            <FormWithBtn
                              name='category_name'
                              placeholder={t('admin.category.sub-category-ex')}
                              formInitialValue={{ 'category_name': '', 'category_type': CONST.CATEGORY_TYPE.SUB, 'parent_id': child.id }}
                              validateScope={'admin'}
                              validateConfigKey={'category_request'}
                    ...
                    <li className={[styles.flex, styles.mb_8, styles.pl_32].join(' ')}>
                      <FormWithBtn
                        name='category_name'
                        placeholder={t('admin.category.main-category-ex')}
                        formInitialValue={{ 'category_name': '', 'category_type': CONST.CATEGORY_TYPE.MAIN, 'parent_id': category.id }}
                        validateScope={'admin'}
                        validateConfigKey={'category_request'}
                        requestUrl={'/api/admin/categories'}
                        createMethod={createData}
