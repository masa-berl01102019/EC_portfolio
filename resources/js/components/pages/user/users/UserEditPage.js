    ...
    return (
        isLoading ? (
            ...
            <div style={{'width': '50%', 'margin':'0 auto'}}>
                ...
                <form onSubmit={ e => {
                    e.preventDefault();
                    dispatch({type: 'UPDATE', form: formData, url: `/api/user/users/${formData.id}` });
                }}>
                    <div>氏名</div>
                    <div>
                    ...
                    <div style={{'marginTop':'30px'}}>
                        <button onClick={() => {
                            ...
                            if(answer) {
                                // TODO: 同時に非同期の処理を投げていてcleanUp関数でworning出てるとこ後で直す
                                // ログアウトの処理
                                handleLogout();
                                // 削除処理
                                dispatch({type:'DELETE', url:`/api/user/users/${formData.id}`});
                            } 
                        }}>退会する</button>
                    </div>
                </form>
