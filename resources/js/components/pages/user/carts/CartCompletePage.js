...
function CartCompletePage() {
    ...
    return (
        <main className={styles.mt_40}>
            <Heading tag={'h1'} tag_style={'h1'} className={styles.section_title}>{i18next.t('user.cart.complete.title')}</Heading>
            <div className={[styles.flex, styles.justify_center, styles.mb_32].join(' ')}>
                ...
                <Text role='title' className={styles.mrl_8}>▶</Text>
                <Text role='title'>{i18next.t('user.cart.progress-complete')}</Text>
            </div>
            <div className={styles.form_contents_area}>
                <Text className={[styles.mb_32, styles.paragraph].join(' ')}>
                    {i18next.t('user.cart.complete.p1')}<br/>
                    {i18next.t('user.cart.complete.p2')}<br/>
                    {i18next.t('user.cart.complete.p3')}<br/>
                    {i18next.t('user.cart.complete.p4')}
                </Text>
