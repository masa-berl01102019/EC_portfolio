...
function UserEdit(props) {
    ...
    const [{isLoading, errorMessage, data}, dispatch] = DataFetchApi(`/api/admin/users/${props.match.params.id}/edit`, 'get', []);
    ...
    return (
        <div style={{'width': '50%', 'margin': '0 auto'}}>
            ...
            { isLoading ? (
                ...
                <form onSubmit={ e => {
                    // preventDefault()しないとなぜかPOST後に'/api/admin/users/create?userInfo'の形でリクエストが飛んで落ちてしまう。。原因を調べる
                    e.preventDefault();
                    dispatch({type: 'UPDATE', form: userInfo, url: `/api/admin/users/${props.match.params.id}` });
                }}>
                    <div>お名前</div>
                    <div>
